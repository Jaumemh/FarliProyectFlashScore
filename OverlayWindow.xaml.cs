    ...
    public partial class OverlayWindow : Window
    {
                ...
                try
                {
                    ...
                    foreach (var res in results)
                    {
                        if (res != null && _matches.TryGetValue(res.OverlayId ?? res.MatchId!, out var existing))
                        {
                            ...
                            if (res.HomeQuarters != null && res.HomeQuarters.Count > 0)
                            {
                                existing.HomeQuarters = res.HomeQuarters;
                                existing.AwayQuarters = res.AwayQuarters;
                            }
                            anyUpdated = true;
                        }
                    }
        ...
        private async Task<MatchData?> RefreshSingleMatchAsync(MatchData match, CancellationToken token)
        {
            ...
            try
            {
                ...
                return new MatchData
                {
                    ...
                    Html = html,
                    HomeQuarters = ExtractQuarters(document, "home"),
                    AwayQuarters = ExtractQuarters(document, "away")
                };
            }
            catch { return null; }
        }
        ...
        private OverlayDisplayData CreateDisplayData(MatchData md)
        {
            ...
            return new OverlayDisplayData
            {
                ...
                Url = md.Url,
                HomeQuarters = md.HomeQuarters ?? new List<string>(),
                AwayQuarters = md.AwayQuarters ?? new List<string>()
            };
        }

        private List<string> ExtractQuarters(AngleSharp.Dom.IDocument document, string side)
        {
            var quarters = new List<string>();
            var smh = document.QuerySelector(".smh__template");
            if (smh == null) return quarters;

            for (int i = 1; i <= 8; i++)
            {
                var part = smh.QuerySelector($".smh__part.smh__{side}.smh__part--{i}");
...
<body>
    ...
    <script>
        ...
        function renderMatch(match) {{
            ...
            const homeQuarters = match.homeQuarters || [];
            const awayQuarters = match.awayQuarters || [];
            const hasQuarters = homeQuarters.length > 0;

            let quartersHtml = '';
            if (hasQuarters) {{
                const homeQHtml = homeQuarters.map(q => `<span class='quarter-val'>${{q}}</span>`).join('');
                const awayQHtml = awayQuarters.map(q => `<span class='quarter-val'>${{q}}</span>`).join('');
                quartersHtml = `
                <div class='quarters-col'>
                    <div class='quarter-row'>${{homeQHtml}}</div>
                    <div class='quarter-row'>${{awayQHtml}}</div>
                </div>`;
            }}
        ...
        public class OverlayDisplayData
        {
            ...
            public List<string> HomeQuarters { get; set; } = new();
            public List<string> AwayQuarters { get; set; } = new();
        }
    }
}