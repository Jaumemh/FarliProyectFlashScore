...
(function () {
    ...
    function loadTeamIndex() {
        try { return JSON.parse(sessionStorage.getItem(TEAM_INDEX_KEY) || '{}'); } catch (e) { return {}; }
    }
    ...
    function indexTeam(name, href) {
        try {
            ...
            let full;
            if (href.startsWith('http://') || href.startsWith('https://')) { full = href; }
            else if (href.startsWith('www.flashscore.es')) { full = 'https://' + href; }
            else if (href.startsWith('//')) { full = 'https:' + href; }
            else { full = 'https://www.flashscore.es' + (href.startsWith('/') ? href : '/' + href); }
        ...
        try {
            ...
            for (const a of anchors) {
                try {
                    ...
                    if (t) {
                        let fullHref;
                        if (h.startsWith('http://') || h.startsWith('https://')) { fullHref = h; }
                        else if (h.startsWith('www.flashscore.es')) { fullHref = 'https://' + h; }
                        else if (h.startsWith('//')) { fullHref = 'https:' + h; }
                        else { fullHref = 'https://www.flashscore.es' + (h.startsWith('/') ? h : '/' + h); }
    ...
    async function sendToApp(action, data) {
        ...
        if (!response.ok) { throw new Error(`Servidor devolvió ${response.status}`); }
        return response;
    }
    ...
    async function pollForCommands() {
        try {
            ...
            if (json && json.action === 'navigate' && json.href) {
                ...
                json.matchIds.forEach(id => { if (id) handleRemoteRemoveMatch(id); });
    ...
    function getTeamHref(matchElement, participantSelector) {
        try {
            ...
            if (anchor) {
                ...
                if (h) {
                    let full;
                    if (h.startsWith('http://') || h.startsWith('https://')) { full = h; }
                    else if (h.startsWith('www.flashscore.es')) { full = 'https://' + h; }
                    else if (h.startsWith('//')) { full = 'https:' + h; }
                    else { full = 'https://www.flashscore.es' + (h.startsWith('/') ? h : '/' + h); }
    ...
    function getMatchData(matchElement) {
        ...
        if (linkElement) {
            ...
            if (href) { matchUrl = href.startsWith('http') ? href : `https://www.flashscore.es${href}`; }
        }
        ...
        const extractFlag = (side) => {
            ...
            const logoSels = [`.event__logo--${side}`, `[class*="logo--${side}"]`];
            ...
            const parentSels = [`.event__participant--${side}`, `[class*="participant--${side}"]`];
        ...
        return {
            matchId, matchMid, overlayId: matchId,
    ...
    const SPORT_SLUG_MAP = {
        ...
        'futsal': 'Futsal', 'golf': 'Golf', 'mma': 'MMA',
        'motorsport': 'Motorsport',
        'ciclismo': 'Ciclismo', 'cycling': 'Ciclismo',
        'waterpolo': 'Waterpolo', 'water-polo': 'Waterpolo',
        'badminton': 'Bádminton', 'snooker': 'Snooker',
        'tenis-de-mesa': 'Tenis de Mesa', 'table-tennis': 'Tenis de Mesa',
        'boxeo': 'Boxeo', 'boxing': 'Boxeo',
        'padel': 'Pádel', 'floorball': 'Floorball',
        'bandy': 'Bandy', 'rugby-league': 'Rugby League',
        'afl': 'AFL', 'netball': 'Netball',
        'pesapallo': 'Pesäpallo', 'kabaddi': 'Kabaddi'
    };
    ...
    function getCompetitionData(headerBody) {
        ...
        let leagueFlagUrl = '';
        try {
            const flagEl = headerBody.querySelector('.headerLeague__flag, [class*="headerLeague__flag"]');
            if (flagEl) {
                // 1. Check background-image (CSS)
                const bg = getComputedStyle(flagEl).backgroundImage;
                if (bg && bg !== 'none') {
                    const m = bg.match(/url\(["']?([^"')]+)["']?\)/);
                    if (m && m[1]) leagueFlagUrl = m[1];
                }
                // 2. Fallback to nested img src
                if (!leagueFlagUrl) {
                    const img = flagEl.querySelector('img');
                    if (img && img.src) leagueFlagUrl = img.src;
                }
            }
            // 3. Last fallback: look for ANY img in the header logo container
            if (!leagueFlagUrl) {
                const logoImg = headerBody.querySelector('.headerLeague__logo img');
                if (logoImg && logoImg.src && !logoImg.src.includes('logo')) {
                    leagueFlagUrl = logoImg.src;
                }
            }
        ...
        allWrappers.forEach(wrapper => {
            ...
            if (wrapperRect.bottom <= matchRect.top) {
                ...
                if (distance < minDistance) {
                    ...
                    if (body) { minDistance = distance; closestWrapper = wrapper; }
                }
            }
        });
    ...
    function createOverlayButton(matchElement) {
        ...
        btn.addEventListener('click', async function (e) {
            ...
            if (!headerWrapper) { alert('No se pudo identificar la competición'); return; }
            ...
            if (isActive) {
                try {
                    await sendToApp('removeMatch', { match: matchData, competition: competitionData });
                ...
                try {
                    await sendToApp('addMatch', { match: matchData, competition: competitionData });
    ...
    async function removeAllMatchesForSport(headerWrapper) {
        ...
        if (!sport) { console.warn('Could not determine sport from header'); return; }
        ...
        while (sibling) {
            ...
            const matchEls = [];
            if (sibling.classList && sibling.classList.contains('event__match')) matchEls.push(sibling);
            if (sibling.querySelectorAll) sibling.querySelectorAll('.event__match').forEach(m => matchEls.push(m));
    ...
    function tryOpenTeamFromParam() {
        try {
            ...
            const iv = setInterval(() => {
                attempts++;
                if (clickTeamOnce()) { clearInterval(iv); return; }
                if (attempts >= maxAttempts) {
                    clearInterval(iv);
                    sessionStorage.removeItem('fc_open_team');
                    sessionStorage.removeItem('fc_open_href');
    ...
    // ===== DOM OBSERVER (only for adding pin buttons) =====
